...
// 获取微信用户授权登录用户信息
import { wxLogin, getguituUserInfo, getWxRedirect } from '@/smart/api/common'
import { setToken, removeToken } from '@/utils/auth'
export async function getguituAuthUserInfo(appId) {
  ...
  if (!code) {
    ...
    if (appId) {
      url = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appId}&redirect_uri=${encodeURIComponent(
        local,
      )}&response_type=code&scope=snsapi_userinfo&state=STATE#guitu_redirect`
    } else {
    ...
    //     `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appid}&redirect_uri=${encodeURIComponent(local)}&response_type=code&scope=snsapi_userinfo&state=1#guitu_redirect`;
  }
  try {
    ...
    if (code) {
      ...
      let { data: dataUser } = await getguituUserInfo()
      removeToken()
...
import { getProCityList } from '@/smart/api/common'
