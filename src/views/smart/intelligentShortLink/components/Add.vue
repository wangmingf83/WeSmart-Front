...
import { getCode } from '@/smart/api/drainageCode/store'
export default {
  components: {},
  props: {
    form: {
  ...
  data() {
    return {
      ...
      selectQrCode: defineAsyncComponent(() => import('@/smart/components/SelectQrCode.vue')),
      selectStaffQrCode: defineAsyncComponent(() => import('@/smart/components/SelectStaffQrCode.vue')),
      component: '',
      qrCodeVisible: false,
      ...
      <template v-else-if="[7, 9].includes(+form.type)">
        ...
        <el-form-item prop="longLink" label="小程序页面路径">
          ...
          <div v-if="!isDetail" class="tips">
            ...
            <a href="https://www.yuque.com/guitu/help/gy4ghv#a1bXG" target="_blank">如何添加小程序页面路径?</a>
          </div>
        </el-form-item>
      </template>
    </el-form>
