<template>
  <doc-tip title="生成客户群的二维码，根据规则自动切换企微客群，支持群满自动建群。" />
  <div>
    <RequestChartTable ref="table" :request="getList">
      <template #query="{ query }">
        <el-form-item label="活码名称" prop="activityName">
          <el-input clearable v-model="query.activityName" placeholder="请输入"/>
        </el-form-item>
        <el-form-item label="入群标签" prop="tagIds">
          <el-input :model-value="tagNames" readonly @click="showSelectTag = true" placeholder="请选择" />
        </el-form-item>
        <!-- <      label="创建人" prop="createBy">
        <el-input v-model="query.createBy" placeholder="请输入"/>
      </>
      <el-form-item label="创建时间">
        <el-date-picker v-model="searchDate" format="YYYY-MM-DD" value-format="YYYYMMDD" type="daterange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期"/>
      </el-form-item> -->
      </template>

      <template #operation="{ selectedIds }">
        <div class="mid-action mb0">
          <el-button type="primary" @click="newCode">新建活码</el-button>
          <div>
            <el-button
              type="primary"
              plain
...
<!--      <template #table="{ data }">-->
      <template #table>
        <el-table-column prop="activityName" label="活码名称" align="center"/>
        <el-table-column prop="codeUrl" label="二维码" align="center" width="130">
          <template #default="{ row }">
            <el-popover placement="bottom" trigger="hover">
              <template #reference><el-image :src="row.codeUrl" class="code-image--small"/></template>
        ...
        <el-table-column label="入群标签" width="180px">
          <template #default="{ row }">
            <TagEllipsis :list="row.tags?.split(',')" emptyText/>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="updateTime" label="最近操作时间" align="center" width="150"/>
        <el-table-column label="操作" align="center" width="220" fixed="right">
          <template #default="{ row }">
            ...
            <!-- <el-dropdown style="margin-left: 10px">
              <el-button text>
                <el-icon-MoreFilled class="el-icon-MoreFilled"/>
              </el-button>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item>
    ...
    <select-tag v-model:visible="showSelectTag" type="1" :selected="tagList" @success="getSelectTag"/>
...
<script>
import { getList, remove, downloadBatch, download, getTableTotal } from '@/smart/api/drainageCode/group'
...
export default {
  ...
  methods: {
    newCode() {
      ...
      this.$router.push({
        path: './add',
        meta: { hidden: true, external: false } // 显式禁用外部跳转
      })
      // const resolved = this.$router.resolve('add')
      // this.$router.push(resolved.href)  // 确保跳转到解析后的完整路径
      // this.$router.push({
      //   path: '/drainageCode/add',
      // })
    },
    getDetail(id) {
