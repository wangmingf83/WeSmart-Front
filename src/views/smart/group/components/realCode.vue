<script>
import { getList, add, remove, update } from '@/smart/api/drainageCode/actual'
...
<template>
  <div>
    ...
    <el-table :data="realCodes" v-loading="loading" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="groupName" label="群名称" align="center"/>
      <el-table-column prop="actualGroupQrCode" label="实际群码" align="center" width="80">
        <template #default="{ row }">
          <el-popover placement="bottom" trigger="hover">
            <template #reference><el-image :src="row.actualGroupQrCode" class="code-image--small"/></template>
            <el-image :src="row.actualGroupQrCode" class="code-image"/>
          </el-popover>
        </template>
      </el-table-column>
      <!-- <el-table-column prop="chatGroupName" label="客户群" align="center"/> -->
      <el-table-column prop="effectTime" label="有效期" align="center"/>
      <el-table-column prop="scanCodeTimesLimit" label="扫码次数限制" align="center"/>
      <el-table-column prop="scanCodeTimes" label="已扫码入群数" align="center"/>
    ...
    <el-dialog title="实际群码" v-model="dialog" append-to-body>
      <el-form :model="form" ref="form" label-width="120px" :rules="rules">
        ...
        <el-form-item label="扫码次数限制" prop="scanCodeTimesLimit">
          <el-input v-model="form.scanCodeTimesLimit" placeholder="请输入次数" type="Number"/>
      ...
      <el-dialog title="如图所示为标准的微信群二维码" v-model="exampleDialog" append-to-body width="400px">
        <div class="example-code-box">
          ...
          <div class="code-box-title">
            <svg-icon icon="user" class="code-user"/>
            实际群名称
          </div>
          <div class="code-content">
            <el-image :src="'@/smart/assets/example/groupCode.png'"/>
          </div>
        </div>
      </el-dialog>
