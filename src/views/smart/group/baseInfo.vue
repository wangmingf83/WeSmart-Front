    ...
    <div class="g-card fxauto">
      <el-form
        ...
        <el-form-item label="活码名称:" prop="activityName">
          <el-input v-model="form.activityName" placeholder="请输入名称" show-word-limit maxlength="15"/>
<!--          <el-tag
            class="fr cp"
            v-if="isDetail"
            size="large"
        ...
        <el-form-item label="活码客群:" prop="chatIdList">
          <template v-if="!isDetail">
            ...
            <SelectGroup v-model:visible="showSelectModal" :defaults="groupList" @submit="setSelectData"/>
            <div class="sub-des">最多选择五个客群</div>
          </template>

          <TagEllipsis :list="groupList" limit="10" defaultProps="groupName" emptyText/>
        </el-form-item>
        <el-form-item label="入群标签:" prop="tagIds">
          ...
          <TagEllipsis :list="tagList" limit="10" emptyText/>
        </el-form-item>
        <el-form-item label="群满是否自动建群:">
          <el-switch v-model="form.autoCreateRoom" :active-value="1" :inactive-value="0"/>
        ...
        <template v-if="form.autoCreateRoom">
          ...
          <el-form-item label="群起始序号:" prop="roomBaseId">
            <el-input-number v-model="form.roomBaseId" controls-position="right" :min="1"/>
          </el-form-item>
        </template>
      </el-form>
    </div>
...
<script>
import { add, update, getDetail } from '@/smart/api/drainageCode/group'
