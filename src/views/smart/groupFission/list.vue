<template>
  <doc-tip title="通过任务形式，引导老客裂变新客加群，实现客群增长。" />
  <div>
    <el-form ref="queryForm" :model="query" inline class="top-search" label-position="left" label-width="">
      <el-form-item label="" prop="fassionName">
        <el-input clearable v-model="query.fassionName" placeholder="请输入任务名称"/>
      </el-form-item>
      <el-form-item label="" prop="fassionState">
        <el-select :popper-append-to-body="false" v-model="query.fassionState" placeholder="请选择任务状态">
          <el-option label="全部任务状态" value=""/>
          <el-option v-for="(item, index) in statusType" :key="index" :label="item.name" :value="item.key"/>
        </el-select>
      </el-form-item>
      <el-form-item label="">
    ...
    <div class="g-card">
      ...
      <el-table :data="list" v-loading="loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="fassionName" label="任务名称" align="center" width="120" fixed="left"/>
        ...
        <el-table-column prop="updateTime" label="最近操作时间" align="center" width="180"/>
