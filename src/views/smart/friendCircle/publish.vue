    ...
    <el-form
      ...
      <div class="g-card">
        <el-form-item label="任务名称" prop="name" v-if="![2, 3].includes(firendType)">
          <el-input v-model="form.name" placeholder="请输入任务名称" maxlength="20" show-word-limit/>
        </el-form-item>
          ...
          <el-radio-group v-model="form.sendType" v-else>
            ...
            <el-radio :label="2">
              ...
              <el-popover placement="top" trigger="hover">
                ...
                <div>需要成功在手机端手动发送，但同一条朋友圈对相同客户会展示每个成员发送的朋友圈</div>
              </el-popover>
            </el-radio>
          </el-radio-group>
        </el-form-item>
...
<!--       <FriendCircleContent ref="friendCircleContent" :data="form"/> -->
      <AddMaterial
        :moduleType="4"
        @update="onBackStep"
        @submit="submit"
    ...
    <!-- 选择标签弹窗 -->
    <SelectTag v-model:visible="dialogVisibleSelectTag" :selected="likeTagList" @success="submitSelectTag"/>
    <SelectTag
...
<script>
import { addMoments, numMoments, getDetail } from '@/smart/api/circle'
import AddMaterial from '@/smart/components/ContentCenter/AddMaterial'
import FirendContent from './components/FirendContent.vue'
import moment from 'moment'
import { getList } from '@/smart/api/customer/tag'
import { getCustomerList } from '@/smart/api/groupMessage'
...
export default {
  ...
  components: {
    AddMaterial,
    FirendContent,
    // FriendCircleContent: defineAsyncComponent(() => import('@/smart/components/FriendCircleContent')),
  },
  data() {
    return {
      pickerOptions: {
      ...
      rules: {
        ...
        content: [{ required: true, message: '请输入朋友圈内容', trigger: 'blur' }],
  ...
  methods: {
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.executeTime && typeof this.form.executeTime !== 'string') {
            ...
            const executeTime = new Date(this.form.executeTime);
            this.form.executeTime = moment(executeTime).format('YYYY-MM-DD HH:mm')
          }
          if (this.form.executeEndTime && typeof this.form.executeEndTime !== 'string') {
            ...
            const executeEndTime = new Date(this.form.executeEndTime);
            this.form.executeEndTime = moment(executeEndTime).format('YYYY-MM-DD HH:mm')
          }
          console.log("===1===", this.form.executeTime)
          console.log("===1===", this.form.executeEndTime)
    ...
    filPicType(file) {
      ...
      if (type === 'pdf') {
        return lwConfig.lwCommon.DEFAULT_H5_PDF
      } else if (['doc', 'docx'].includes(type)) {
        return lwConfig.lwCommon.DEFAULT_H5_WORDE
      } else if (['ppt', 'pptx', 'pps', 'pptsx'].includes(type)) {
        return lwConfig.lwCommon.DEFAULT_H5_PPT
      } else {
        return ''
      }
    },
    ...
    //         other: data.picUrl ? data.picUrl : lwConfig.lwCommon.DEFAULT_H5_TP,
    //         title: data.title,
    //       }
    //       this.form.otherContent.push(arr)
    //       break
    ...
    //         annexType: 'link',
    //         annexUrl: linkUrl,
    //         other: data.picUrl ? data.picUrl : lwConfig.lwCommon.DEFAULT_H5_ART,
    //         title: data.title,
    //       }
    //       this.form.otherContent.push(arr)
    //       break
