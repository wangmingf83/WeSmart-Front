<template>
  <doc-tip title="可对企业的朋友圈资源进行统一运营，互动联动功能进行自动打标签，统计成员朋友圈执行效率功能等。朋友圈仅可发送图片、图文、海报和视频素材" />
  <div>
    <el-form :model="query" label-position="left" ref="queryForm" :inline="true" label-width="70px" class="top-search">
      <el-form-item prop="name">
        <el-input v-model="query.name" placeholder="请输入任务名称"/>
      </el-form-item>
      <el-form-item prop="createBy">
        <el-input v-model="query.createBy" placeholder="请输入创建人"/>
      </el-form-item>
      <el-form-item prop="sendType">
        <!-- <el-input v-model="query" placeholder="请输入创建人"/> -->
        <el-select clearable v-model="query.sendType" placeholder="请选择发送方式">
          <el-option v-for="(item, index) in sendType" :key="index" :label="item.label" :value="item.value"/>
        </el-select>
      </el-form-item>
    ...
    <div class="g-card">
      <div class="mid-action">
        ...
        <div>
          ...
          <el-popover placement="top" trigger="hover">
            <template #reference>
              <el-icon-QuestionFilled class="el-icon-QuestionFilled mr5"/>
            </template>
      ...
      <el-table ref="table" v-loading="loading" :data="tableData">
        ...
        <el-table-column show-overflow-tooltip prop="finishRate" label="完成率"/>
...
import { getEnterpriceList, syncHMoments, cancelMoments, reminderMoments } from '@/smart/api/circle'
export default {
  name: 'enterprise',
  components: {},
  data() {
