<template>
  <div>
    <!-- <el-button slot="append" circle icon="el-icon-back" @click="$router.back()"/>返回 -->
    <div class="g-card flex aic mb20">
      <el-image class="avatar" :src="customer.avatar" fit="fill"/>
      <div class="ml10">
        <div>
        ...
        <div class="g-card">
          ...
          <el-tabs model-value="0">
            <!-- <el-tab-pane
              ...
              data-id="0"
            >
              <info-tab :opportunityStageList="opportunityStageList" :trackUsers="customer.trackUsers"/>
            </el-tab-pane> -->
            <el-tab-pane
              ...
              <info-tab :stageList="opportunityStageList" :userId="item.trackUserId"/>
            </el-tab-pane>
          </el-tabs>
        </div>
      </div>
        ...
        <el-card class="g-card" style="padding: 0">
          <template #header>
            <div class="g-card-title fcbw">
              <span class="card-title">客户朋友圈轨迹</span>
              <ButtonSync class="btn-sync" :lastSyncTime="lastSyncTime" @click="sync">同步轨迹</ButtonSync>
            </div>
          </template>
          <div class="flex track-tab-wrap mb15">
            ...
            <el-select v-model="active" placeholder="请选择" @change="changeTrack">
              <el-option v-for="(value, type) of trajectoryType" :key="type" :label="value" :value="type"/>
            </el-select>
          </div>
...
<script>
import { updateBirthday, getDetail, getSummary, getFollowUpRecord, getCustomerInfoByUserId } from '@/smart/api/customer'
import { getList } from '@/smart/api/salesCenter/businessConver.js'

import { dictAddType, dictJoinGroupType, dictTrackState } from '@/smart/utils/dictionary'
