<template>
  <doc-tip title="可自定义修改企业客户的画像信息项供企业个性化使用。" />
  <div>
    <el-form ref="queryForm" :model="query" inline class="top-search" label-position="left">
      <el-form-item label="">
        <el-input v-model="query.labelName" placeholder="请输入信息项名称"/>
      </el-form-item>
      <el-form-item label="">
        <el-select :popper-append-to-body="false" v-model="query.type" placeholder="请选择信息项类型">
          <el-option v-for="(item, index) in infoType" :key="index" :label="item.name" :value="item.key"/>
        </el-select>
      </el-form-item>
      <el-form-item label="">
    ...
    <div class="g-card">
      ...
      <el-table ref="table" row-key="id" :data="list" v-loading="loading">
        ...
        <el-table-column label="信息项名称" align="center" prop="labelName"/>
    ...
    <!-- 选择标签弹窗 -->
    <SelectTag ref="selectTag" v-model:visible="dialogVisible" title="选择标签" @success="querySelectFn"/>
...
import { getList, addField, updateField, deleteField, multiField } from '@/smart/api/customer/defineAttr.js'
export default {
