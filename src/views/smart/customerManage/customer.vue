<template>
  <doc-tip title="用于查看当前企业所有的客户列表及详细信息，支持对客户进行打标签。" />
  <div v-loading="loading">
    <el-form ref="queryForm" :inline="true" :model="query" label-width="70px" class="top-search">
      ...
      <el-form-item label="客户类型" prop="customerType">
        <el-select :popper-append-to-body="false" v-model="query.customerType" placeholder="请选择">
          <el-option v-for="(item, index) in dictCustomerType" :key="index" :label="item" :value="index"/>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="跟进阶段" prop="trackState">
        <el-select :popper-append-to-body="false" v-model="query.trackState" placeholder="请选择">
          <el-option
            v-for="(item, index) in dictTrackStateOptions"
            :key="index"
            :label="item.label"
            :value="item.value"/>
        </el-select>
      ...
      <el-form-item label="商机阶段" prop="opportunityStage">
        <el-select :popper-append-to-body="false" v-model="query.opportunityStage" placeholder="请选择">
          <el-option
            v-for="(item, index) in opportunityStageList"
            :key="index"
            :label="item.stageKey"
            :value="item.id"/>
        </el-select>
      ...
      <el-form-item label="添加方式" prop="addMethod">
        <el-select :popper-append-to-body="false" v-model="query.addMethod" placeholder="请选择">
          <el-option v-for="(item, index) in dictAddType" :key="index" :label="item" :value="index"/>
        </el-select>
      </el-form-item>
    ...
    <div class="g-card">
      ...
      <el-table
        ...
        <el-table-column label="客户" prop="customerName" header-align="center" align="" width="180">
          <template #default="{ row }">
            <div class="cp flex aic" @click="goRoute(row)">
              <el-image class="avatar" :src="row.avatar" fit="fill"/>
              <div class="ml10">
        ...
        <el-table-column prop="tagNames" label="客户标签" align="center" width="220">
          <template #default="{ row }">
            <TagEllipsis :list="row.tagNames" emptyText="无标签"/>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="opportunityStage" label="商机阶段" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.opportunityStage">
              {{ setValue(row.opportunityStage) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="trackState" label="跟进阶段" align="center">
          <template #default="{ row }">
            <el-tag
              v-if="dictTrackState[row.trackState]"
    ...
    <el-dialog title="选择员工" v-model="dialogVisibleExtend" :close-on-click-modal="false">
      <SelectStaffForm ref="SelectStaffForm" v-model:selected="extendStaff"/>
      <template #footer>
        <div>
...
<script>
import * as api from '@/smart/api/customer'
import { getList as getListTag } from '@/smart/api/customer/tag'
import AddTag from '@/smart/components/AddTag.vue'

import DeleteTag from '@/smart/components/DeleteTag'
import { dictAddType, dictTrackState, dictTrackStateOptions } from '@/smart/utils/dictionary'
import { getList } from '@/smart/api/salesCenter/businessConver.js'
...
export default {
  ...
  data() {
    return {
      ...
      query: {
        ...
        trackState: '', //1:待跟进,2:跟进中，3已赢单，4已输单,5:已流失,6:无意向
        opportunityStage: null, // 商机阶段
  ...
  methods: {
    setValue(data) {
      let str = ''
      this.opportunityStageList.forEach((dd) => {
        if (dd.id == data) {
          str = dd.stageKey
        }
      })
    ...
    getStage() {
      getList().then((res) => {
        this.opportunityStageList = res.data
        //console.log('---ddddd---', this.opportunityStageList)
      })
    },
    getList(page) {
      ...
      const queryParams = {
        ...this.query,
        opportunityStage: this.query.opportunityStage ? Number(this.query.opportunityStage) : null
      }

      api
        .getListNew(queryParams)
    ...
    goRoute(row) {
      ...
      console.log('===1====', lwConfig.lwCommon.CUSTOMER_DETAIL_ROUTE_NAME)
      this.$router.push({
        path: lwConfig.lwCommon.CUSTOMER_DETAIL_ROUTE_NAME,
